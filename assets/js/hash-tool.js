...
function copyHash(id) {
  ...
  navigator.clipboard.writeText(text).then(function() {
    ...
    n.innerHTML = '<i class="fas fa-check-circle"></i> <span>已复制到剪贴板</span>';
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var hashMD5 = document.getElementById('hashMD5');
  var hashSHA1 = document.getElementById('hashSHA1');
  var hashSHA256 = document.getElementById('hashSHA256');
  var hashSHA512 = document.getElementById('hashSHA512');
  ...
  async function computeHashes(text) {
    ...
    ['SHA-1', 'SHA-256', 'SHA-512'].forEach(function(algo) {
      crypto.subtle.digest(algo, data).then(function(buffer) {
        ...
        if (algo === 'SHA-1') hashSHA1.textContent = hex;
        else if (algo === 'SHA-256') hashSHA256.textContent = hex;
        else hashSHA512.textContent = hex;
      });
    });
  ...
  input.addEventListener('input', function() {
    ...
    debounceTimer = setTimeout(function() { computeHashes(input.value); }, 200);
  });
